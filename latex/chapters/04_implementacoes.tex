...
Antes de comentar das implementações, ressaltamos que os algoritmos foram elaborados no formato de Iteradores. Dado que Grafos são estruturas diversas e que podem ser representadas de múltiplas maneiras, a criação de iteradores para cada algoritmo pode ser aplicada no lugar de funções específicas para cada ED, onde o iterador não precisa conhecer detalhes de implementação da estrutura para funcionar. Além disso, o iterador fornece a execução de qualquer algoritmo passo a passo e pode armazenar dados em sua própria estrutura, o que é bastante útil para armazenar informações úteis que não pertencem originalmente ao grafo, como uma lista de vértices visitados. Por fim, em Rust, iteradores tem a característica de serem abstrações de zero custo, ou seja, não geram custo adicional em tempo de execução, sendo similares a loops convencionais.
